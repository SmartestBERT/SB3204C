...
comms->write8(i2cAddress,ledOut0,&ledData0,1); //Lane0&Lane1 turn-on
comms->write8(i2cAddress,ledOut1,&ledData1,1); //Lane2&lane3 turn-on
...
void TLC59108::ChangPGLedStatus(int lane, bool laneOn)
{
    ...
    if(BertModel::UseFourChanPGMode())
    {
           ...
           qDebug()<<"ledData0" <<ledData0<<"ledData1"<<ledData1;
     }
    else
    {
...
void TLC59108::edLedFlash(int lane, bool flagEDStop, bool edErroflashOn)
{
...
void TLC59108::StopEDLed()
{
//    Sleep(500);
    ledOn[1] = false;
    ledOn[3] = false;
    ChangEDLedStatus();
    qDebug() << "All ED LEDs Stopped ";
}

void TLC59108::StartEyeScanLed(int lane)
{
    ledOn[lane] = true;
    if((ledOn[1] ==true)&&(ledOn[3] == false)) ledData1 = 0x05;
    else if((ledOn[1] ==false)&&(ledOn[3] == true)) ledData1 =0x50;
    else if((ledOn[1] ==true)&&(ledOn[3] == true)) ledData1 = 0x55;
    comms->write8(i2cAddress,ledOut1,&ledData1,1);
    qDebug() <<"EyeScan LED is on" <<"lane" <<lane;
}
...
void TLC59108::ChangEDLedStatus()
{
        ...
        else if(ledOn[1] == true && ledOn[3] == false)  //ED1 on and ED2 off
        {
         ...
         qDebug() <<"ledOn[1]"<<ledOn[1]<<"  Green[1]"<<Green[1]<<"  ledOn[3]"<<ledOn[3]<<"  Green[3]"<<Green[3];

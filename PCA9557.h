...
class PCA9557 : public BertComponent
{
    Q_OBJECT

public:

    PCA9557(I2CComms *comms, const uint8_t i2cAddress, const int deviceID);

    static bool ping(I2CComms *comms, const uint8_t i2cAddress);
    void getOptions();
    int init();

#define PCA9557_SIGNALS \
    void LMXLockDetect(int deviceId, bool isLocked);

#define PCA9557_SLOTS \
    void SelectTriggerDivide(int index);    \
    void SetEEPROMWriteEnable(bool enable); \
    void ReadLMXLockDetect();

#define PCA9557_CONNECT_SIGNALS(CLIENT, PCA9557) \
    connect(PCA9557, SIGNAL(LMXLockDetect(int, bool)),    CLIENT,  SLOT(LMXLockDetect(int, bool)));    \
    connect(CLIENT,  SIGNAL(SelectTriggerDivide(int)),    PCA9557, SLOT(SelectTriggerDivide(int)));    \
    connect(CLIENT,  SIGNAL(SetEEPROMWriteEnable(bool)),  PCA9557, SLOT(SetEEPROMWriteEnable(bool)));  \
    connect(CLIENT,  SIGNAL(ReadLMXLockDetect()),         PCA9557, SLOT(ReadLMXLockDetect()));         \
    BERT_COMPONENT_CONNECT_SIGNALS(CLIENT, PCA9557)
